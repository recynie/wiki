...
数理统计学之所以重要，是因为现实中的总体往往无法直接穷尽观察，我们只能通过样本去理解整体。它为数据分析、科学实验、质量控制与机器学习提供了从“观察现象”走向“可靠推断”的理论基础。

数理统计学是概率论的逆问题——在已知总体分布形式（或假设其形式）的前提下，研究如何利用样本数据对总体参数进行推断。本笔记系统性地介绍数理统计学的核心理论体系。
...
1. **抽样分布基础** $ightarrow$ 理解从总体到样本的随机性映射，是整个数理统计的起点。
2. **三大抽样分布** $ightarrow$ 基于正态总体的衍生分布，构成假设检验的理论基石。
3. **次序统计量** $ightarrow$ 样本排序后的特性，在非参数统计中有重要应用。
4. **充分性与数据压缩** $ightarrow$ 如何不损失信息地提取样本特征，引出指数族分布。
5. **点估计** $ightarrow$ 利用样本给出参数的具体数值估计。
6. **区间估计** $ightarrow$ 给出参数的可能范围。
7. **假设检验** $ightarrow$ 基于样本对总体断言进行决策。
8. **贝叶斯推断** $ightarrow$ 将参数视为随机变量的哲学范式。
...
本笔记主要聚焦于推断统计的理论体系，包括参数估计（点估计、区间估计）和假设检验两大支柱。
